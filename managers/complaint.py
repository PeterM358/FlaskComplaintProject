...
class ComplaintManager:
    ...
    def create(data, user):
        ...
        try:
            ...
            db.session.flush()
            # TODO create countries in models
    ...
    def reject(complaint_id):
        transaction_id = TransactionModel.query.filter_by(complaint_id=complaint_id).first()
        wise.cancel_transfer(transaction_id)
        ComplaintModel.query.filter_by(id=complaint_id).update({"status": ComplaintState.rejected})

    @staticmethod
    def issue_transaction(amount, full_name, iban, country, complaint_id):
        quote_id = wise.create_quote("EUR", "EUR", amount)
        recipient_id = wise.create_recipient(full_name, iban, country)
